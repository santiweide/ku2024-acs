...
public class BookStoreHTTPProxy implements BookStore {
	...
	public List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		// Validate input
		if (numBooks <= 0) {
			throw new BookStoreException("Number of books must be greater than zero.");
		}

		// Encode the number of books as a query parameter
		String urlEncodedNumBooks = URLEncoder.encode(Integer.toString(numBooks), StandardCharsets.UTF_8);

		// Construct the URL for the GETTOPRATES endpoint
		String urlString = serverAddress + "/" + BookStoreMessageTag.GETTOPRATES + "?"
				+ BookStoreConstants.BOOK_NUM_PARAM + "=" + urlEncodedNumBooks;

		// Create a GET request and perform the HTTP exchange
		BookStoreRequest bookStoreRequest = BookStoreRequest.newGetRequest(urlString);
